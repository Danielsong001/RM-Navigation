...
void ConstraintSet::LoadParam() {
  ...
  std::string file_name = camera_filename_;
//      "//home//yingtian//RoboRTS//modules//perception//detection//armor_detection//constraint_set//config//constraint_set.prototxt";
...
ErrorInfo ConstraintSet::DetectArmor(double &distance, double &pitch, double &yaw) {
  ...
  if (!src_img_.empty()) {
    ...
    if (!armors.empty()) {
      ...
      CalcControlInfo(final_armor, distance, pitch, yaw, 19);
    }
    lights.clear();
    armors.clear();
  } else {
...
void ConstraintSet::DetectLights(const cv::Mat &src, std::vector<cv::RotatedRect> &lights) {
  ...
  //TODO(noah.guo): param
  cv::threshold(gray_img_, binary_brightness_img, 180, 255, CV_THRESH_BINARY);
  cv::dilate(binary_brightness_img, binary_brightness_img, element, cv::Point(-1, -1), 1);
  if (enable_debug_)
    cv::imshow("binary_brightness_img_after_dilate", binary_brightness_img);
  //TODO(noah.guo): param
  float thresh;
  ...
  cv::erode(binary_color_img, binary_color_img, element, cv::Point(-1, -1), 1);
  if (enable_debug_)
    cv::imshow("binary_color_img_after_erode", binary_color_img);
  element = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3));
  cv::dilate(binary_color_img, binary_color_img, element, cv::Point(-1, -1), 1);
  if (enable_debug_)
    cv::imshow("binary_color_img_after_dilate", binary_color_img);
...
void ConstraintSet::CalcControlInfo(const ArmorInfo & armor,
  ...
  //std::cout<<"CalcControlInfo"<<std::endl;
}
