...
class ArmorDetectionNode : public rrts::common::RRTS {
 public:
  ...
  //gimble_call_back
  void GimbleRealInfo(const messages::EnemyPosConstPtr &msg);//lzh
  //Kalman Filter reset
  void KalmanReset();//LZH
 ...
 private:
  ...
  //lzh
  bool received_gimbal_info_state = false;
  float dis_record,pi_record,ya_record;
  uint16_t gimbal_restart = 0;

  //ROS
  ros::NodeHandle nh_;
  ros::Publisher enemy_info_pub_;
  ros::Subscriber gimbal_info_pub_;//lzh
  actionlib::SimpleActionServer<messages::ArmorDetectionAction> as_;
  messages::EnemyPos enemy_pos;
  messages::EnemyPos gimbal_info; //lzh
